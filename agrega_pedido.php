                    ...
                    if ($ingreso_neto_solicitante >= 50000) 
                    {
                        if ($monto_solicitado <= 200000) 
                        {
                            ...
                            if($result)
                            {
                                ...
                                $sql_idNew="SELECT MAX(id) AS id FROM pedidos"; //obtenemos el ultimo id ado
                                $result_id_pedido = mysqli_query($dbConn,$sql_idNew);

                                if ($row_id_pedido = mysqli_fetch_row($result_id_pedido)) 
                                {
                                    $idMax = trim($row_id_pedido[0]);
                                }
                                
                                echo "<script> alert('FELICIDADES!: SE HA GENERADO EL FOLIO".$idMax."'); window.location.href='pedidos.php'; </script>";
                                die();
                            }
                            else{
                                echo "<br>error";
